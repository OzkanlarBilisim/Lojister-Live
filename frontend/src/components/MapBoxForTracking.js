...
import { getAdvertisementRouteRequest } from '../api/controllers/client-advertisiment-controller';
...
import { getDriverLastPositionRequest } from '../api/controllers/vehicle-controller';
...
import TransportInfoForTrackingMap from './TransportInfoForTrackingMap';
import { getTransportProcessByTransportIdRequest } from '../api/controllers/client-transport-process-controller';
import { getRouteRequest } from '../api/controllers/map-controller';
import * as turf from '@turf/turf';
import RequestError from './RequestError';
import NoneContent from './NoneContent';
import { useTranslation } from 'react-i18next';

const MapBoxForTracking = ({ scrollControl, transports, getAds, adsLoading, filterValues, setFilterValues, sortValues, setSortValues, defaultFilterValue }) => {
    ...
    const closeClick = () => {
        setTransportProcess(null);
        setDriverLastPosition(null);
        setCoordinates([]);
        setCompletedCoordinates([]);
        setDrawCompletedRoute(false);
    ...
    const [transportProcess, setTransportProcess] = useState(null)
    ...
    const [driverLastPosition, setDriverLastPosition] = useState(null);
    const [drawCompletedRoute, setDrawCompletedRoute] = useState(false)



    const onSelectTransport = async (transportId) => {
        setIsLoading(true);
        closeClick();
        try {
            ...
            if (res) {
                ...
                getAdRoute({ transportId: transportId, transportProcessId: res.data.id })
            }
        } catch (error) {
            AntNotification({ type: "error", message: t("Taşıma detayları getirilirken hata meydana geldi.") });
        }
    }

    //get route for selected ad
    const getAdRoute = async ({ transportId, transportProcessId }) => {
        try {
            let res = await getAdvertisementRouteRequest(transportId);
            if (res) {
                setCoordinates(res.data)
                getDriverLastPosition({ transportProcessId: transportProcessId, routeCoordinates: res.data });
            }
        } catch (error) {
            RequestError(error);
    ...
    //get driver for selected ad
    const getDriverLastPosition = async ({ transportProcessId, routeCoordinates }) => {
        setIsLoading(true)
        let parsedCoordinates = routeCoordinates.map(val => [val.longitude, val.latitude])
        try {
            let res = await getDriverLastPositionRequest(transportProcessId);
            if (res) {
                var line = turf.lineString(parsedCoordinates);
                var pt = turf.point([res.data.longitude, res.data.latitude]);
                var snapped = turf.nearestPointOnLine(line, pt, { units: 'meters' });
                var from = turf.point(snapped.geometry.coordinates);
                var to = turf.point([res.data.longitude, res.data.latitude]);
                var options = { units: 'meters' };
                var distance = turf.distance(from, to, options);

                if (distance < 450) {
                    res.data.latitude = snapped.geometry.coordinates[1];
                    res.data.longitude = snapped.geometry.coordinates[0];
                    setDriverLastPosition({ ...res.data });
                    setDrawCompletedRoute(true);
                } else {
                    setDriverLastPosition(res.data);
                    setIsLoading(false);
                }
    ...
    //get completed route for selected ad
    const getCompletedRoute = async () => {
        try {
            let res = await getRouteRequest({
                endingLat: driverLastPosition.latitude,
                endingLng: driverLastPosition.longitude,
                startingLat: transportProcess.acceptedClientAdvertisementBid.clientAdvertisement.startingAddress.lat,
                startingLng: transportProcess.acceptedClientAdvertisementBid.clientAdvertisement.startingAddress.lng,
            });
            if (res) {
                setCompletedCoordinates(res.data);
            }
        } catch (error) {
            message.error(t('Tamamlanmış rota bulunamadı.'));
            setCompletedCoordinates([]);
        }
        setIsLoading(false)
    }
    ...
    useEffect(() => {
        if (drawCompletedRoute && driverLastPosition) {
            getCompletedRoute()
        }
    }, [drawCompletedRoute, driverLastPosition])

    /*  useEffect(() => {
         if (transportProcess && driverLastPosition) {
             getCompletedRoute();
         }
     }, [driverLastPosition]) */
    ...
    return (
        <>
            <div className="map-transport active" id="map-transport">
                <button onClick={() => handleClickShowHideAds()} className="map-transport-show-hide-btn" id="map-transport-show-hide-btn">
                    <i className="bi bi-chevron-right"></i>
                    <i className="bi bi-x"></i>
                </button>
                <div className="map-transport-container">
                    <div className="map-transport-header">
                        <p className="map-transport-info">{t("Taşımada olan")}<b> {transports.length} </b>{t("ilan arasından seçim yapın!")}</p>
                    ...
                    <div className="map-transport-list">
                        {
                            transports.length > 0 === !adsLoading ?
                                transports.map((val, i) => (
                                    <TransportCard
                                        onClick={() => {
                                            onSelectTransport(val.id);
                                            handleClickShowHideAds();
                                        }}
                                        isActive={val.id === transportProcess?.acceptedClientAdvertisementBid?.clientAdvertisement?.id}
                                        key={i}
                                        data={val}
                                    />
                                )) : adsLoading ?
            ...
            <div style={{ width: "100%", height: "100%", position: "relative" }}>
                ...
                {
                    transportProcess &&
                    <TransportInfoForTrackingMap
                        closeClick={closeClick}
                        transportProcess={transportProcess}
                        driverLastPosition={driverLastPosition}
                ...
                >
                    ...
                    {
                        transportProcess && driverLastPosition &&
                        <Marker
                    ...
                    }
                    {
                        ...
                        transports.map((v, i) => {
                            ...
                            return (
                                <Marker
                                    key={i}
                                    onClick={() => x.length > 1 ? clickSameMarkers(v) : onSelectTransport(v.id)}
                                    longitude={lng}
                                    latitude={lat}
                                    anchor="bottom"
                                >
                    ...
                    <Source
                        ...
                        <Layer
                            ...
                            paint={{ "line-color": "#3B97D2", "line-width": 2, "line-dasharray": [5, 3], }}
                        />
                    </Source>
                    ...
                    <Source
                        ...
                        data={{
                            ...
                            geometry: {
                                type: "LineString",
                                coordinates: completedCoordinates.map(val => [val.longitude, val.latitude]),
                            }
                        }}>
                        <Layer
                            type="line"
...
export default MapBoxForTracking;
...
const SortTransports = ({ sortValues, setSortValues }) => {
    ...
    return (
        <div className="map-sort-transport">
            ...
            <CustomButton style={{ justifyContent: "flex-start" }} onClick={() => setSortValues("clientAdvertisement.adStartingDate")} color={sortValues === "clientAdvertisement.adStartingDate" ? "primary-light" : "var(--white)"} >{t("Yükleme tarihine göre")}</CustomButton>
            <CustomButton style={{ justifyContent: "flex-start" }} onClick={() => setSortValues("clientAdvertisement.adDueDate")} color={sortValues === "clientAdvertisement.adDueDate" ? "primary-light" : "var(--white)"} >{t("Teslimat tarihine göre")}</CustomButton>
        </div>
    )
}