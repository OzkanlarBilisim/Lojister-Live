...
import { updateAbroudBidDenied, updateAbroudStatusUpdateApproved } from '../api/controllers/client-advertisiment-bid-controller';
...
const DetailBidsAbroud = ({ data, user, updatePage, transportId }) => {
    ...
    const CardClient = ({ cardData }) => {
        ...
        const handleOk = async () => {
            ...
            try {
                let res = await updateAbroudStatusUpdateApproved(witchModal.id)
                if (res) {
                    updatePage()
                }
        ...
        const handleNotOk = async () => {
            ...
            var bidId = witchModal.id;
            try {
                let res = await updateAbroudBidDenied(bidId)
                if (res) {
                    updatePage()
                }
        ...
        //         console.log(witchModal.id);
        //         try {
        //             let res = await updateAbroudStatusUpdate({ id: witchModal.id, status: true })
        //             if (res) {
        //                 updatePage()
        //             }
        ...
        //         try {
        //             let res = await updateAbroudStatusUpdate({ id: witchModal.id, status: false })
        //             if (res) {
        //                 updatePage()
        //             }
            ...
            <>
                {
                    cardData?.length < 1 ?
                        ...
                        :
                        cardData?.map((v, i) => (
                            <div key={i} className={v.status === 'DENIED' || v.status === 'AD_CLOSED' || v.status === 'TIMEOUT' ? 'container-bid-transparent' : v.status === 'APPROVED' || v.status === 'TRANSPORT' || v.status === 'COMPLETED' ? 'container-bid-approved' : 'container-bid'}>
                                <div className='card'>
                                    <div className='left'>
                                        <div className='price'>
                                            <p><b>{v.bid} { t("$")}</b></p>
                                        ...
                                        <div className='company' onClick={() => { openCommentModal(v?.companyId) }}>
                                            <div className='text'>
                                                {v?.companyName ?
                                                    v?.companyName.split(' ').map(v =>
                                            ...
                                            <div className='star'>
                                                ...
                                                <div className='text'> {v?.companyRate ? parseFloat(v?.companyRate).toFixed(1) <= 5? parseFloat(v?.companyRate).toFixed(1): "?" : "?"}</div>
                                            </div>
                                        </div>
                                    </div>
                                    {v.status === 'DENIED' || v.status === 'AD_CLOSED' ?
                                        <div>
                                            {t("Reddedildi")}
                                        </div>
                                        :
                                        v.status === 'APPROVED' || v.status === 'TRANSPORT' || v.status === 'COMPLETED' || v.status === 'PAYMENT_SUCCESSFUL' ?
                                ...
                                <div className='bid-info'>
                                    ...
                                    {v.explanation ?
                                        <Tooltip placement="bottom" title={v.explanation}>
                                            <div className={v.status === 'APPROVED' || v.status === 'TRANSPORT' || v.status === 'COMPLETED' ? 'bid-explanation-white' : 'bid-explanation'} >
                ...
                >
                   <p>{t("Teklifi kabul etmek istediğinize emin misiniz?")}</p>
                </Modal>
                <Modal
                    title={<img className='image' src='../../assets/img/warning.svg' alt="" />}
                    ...
                    cancelText={t("İptal Et")}
                >
                    <p>{t("Teklifi reddetmek istediğinize emin misiniz?")} </p>
                </Modal>
...
export default DetailBidsAbroud;