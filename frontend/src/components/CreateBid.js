...
import { createClientAdvertisimentBidRequest } from '../api/controllers/client-advertisiment-bid-controller';
...
const CreateBid = ({ visible, setVisible, transportId, revision, transport }) => {
    ...
    const onSubmit = async () => {
        setIsLoading(true)
        try {
            let res = await createClientAdvertisimentBidRequest({ clientAdvertisimentId: transportId, bidForm: bidForm });
            if (res) {
                handleCancel();
