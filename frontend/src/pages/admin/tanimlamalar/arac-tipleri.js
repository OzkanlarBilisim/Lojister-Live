...
import { createVehicleTypeRequest, deleteVehicleTypeRequest, getVehicleTypeActivateRequest, getVehicleTypeHideRequest, getVehicleTypesRequest } from '../../../api/controllers/vehicle-type-controller';
import AntNotification from '../../../components/AntNotification';
import RequestError from '../../../components/RequestError'

function AdminAracTipleri() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createVehicleTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteVehicleTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getVehicleTypeHideRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getVehicleTypeActivateRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminAracTipleri;