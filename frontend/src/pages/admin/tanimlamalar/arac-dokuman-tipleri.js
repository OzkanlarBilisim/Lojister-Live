...
import { createVehicleDocumentTypeRequest, deleteVehicleDocumentTypeRequest, getActivateVehicleDocumentTypeByIdRequest, getAllVehicleDocumentTypeRequest, getHideVehicleDocumentTypeByIdRequest } from '../../../api/controllers/vehicle-document-type-controller';

const AdminAracDokumanTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createVehicleDocumentTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteVehicleDocumentTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getHideVehicleDocumentTypeByIdRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getActivateVehicleDocumentTypeByIdRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminAracDokumanTipleri;