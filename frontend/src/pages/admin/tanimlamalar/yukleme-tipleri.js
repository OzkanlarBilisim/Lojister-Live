...
import { createLoadTypeRequest, deleteLoadTypeRequest, getLoadTypeActivateRequest, getLoadTypeHideRequest, getLoadTypesRequest } from '../../../api/controllers/load-type-controller';
import AntNotification from '../../../components/AntNotification';
import RequestError from '../../../components/RequestError'

const AdminYuklemeTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createLoadTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteLoadTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getLoadTypeHideRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getLoadTypeActivateRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminYuklemeTipleri;