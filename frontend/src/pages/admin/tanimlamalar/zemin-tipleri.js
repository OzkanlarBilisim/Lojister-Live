...
import { createTrailerFloorTypeRequest, deleteTrailerFloorTypeRequest, getTrailerFloorTypeActivateRequest, getTrailerFloorTypeHideRequest, getTrailerFloorTypesRequest } from '../../../api/controllers/trailer-floor-type-controller';
import AntNotification from '../../../components/AntNotification';
import RequestError from '../../../components/RequestError'

const AdminZeminTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createTrailerFloorTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteTrailerFloorTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getTrailerFloorTypeHideRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getTrailerFloorTypeActivateRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminZeminTipleri;