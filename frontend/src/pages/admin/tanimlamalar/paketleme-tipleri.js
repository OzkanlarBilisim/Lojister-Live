...
import { createPackagingTypeRequest, deletePackagingTypeRequest, getPackagingTypeActivateRequest, getPackagingTypeHideRequest, getPackagingTypesRequest } from '../../../api/controllers/packaging-type-controller';
import AntNotification from '../../../components/AntNotification';
import RequestError from '../../../components/RequestError'

const AdminPaketlemeTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createPackagingTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deletePackagingTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getPackagingTypeHideRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getPackagingTypeActivateRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminPaketlemeTipleri;