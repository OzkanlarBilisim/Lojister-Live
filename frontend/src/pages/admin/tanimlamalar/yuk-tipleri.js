...
import { createCargoTypeRequest, deleteCargoTypeRequest, getCargoTypeActivateRequest, getCargoTypeHideRequest, getCargoTypesRequest } from '../../../api/controllers/cargo-type-controller';
import AntNotification from '../../../components/AntNotification';
import RequestError from '../../../components/RequestError'

const AdminYukTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createCargoTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteCargoTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getCargoTypeHideRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getCargoTypeActivateRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminYukTipleri;