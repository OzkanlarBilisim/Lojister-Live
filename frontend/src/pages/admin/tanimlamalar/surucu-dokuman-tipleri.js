...
import { createDriverDocumentTypeRequest, deleteDriverDocumentTypeRequest, getActivateDriverDocumentTypeByIdRequest, getAllDriverDocumentTypeRequest, getHideDriverDocumentTypeByIdRequest } from '../../../api/controllers/driver-document-type-controller';

const AdminSurucuDokumanTipleri = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            let res = await createDriverDocumentTypeRequest(form);
            if (res) {
                closeModal();
                fetchTypes();
    ...
    const onDelete = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await deleteDriverDocumentTypeRequest(id);
            if (res) {
                fetchTypes();
            }
        } catch (error) {
    ...
    const onHide = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getHideDriverDocumentTypeByIdRequest(id);
            if (res) {
                fetchTypes();
    ...
    const onActivate = async (id) => {
        setIsSubmitting(true);
        try {
            let res = await getActivateDriverDocumentTypeByIdRequest(id);
            if (res) {
                fetchTypes();
...
export default AdminSurucuDokumanTipleri;