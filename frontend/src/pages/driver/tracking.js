...
import { getMyBidsStatusCargoOntheWayRequest } from '../../api/controllers/client-advertisiment-bid-controller';
import MapBoxForTracking from '../../components/MapBoxForTracking'
import RequestError from '../../components/RequestError';

const DraverTracking = () => {
    ...
    const getAds = async () => {
        setAdsLoading(true)
        try {
            let res = await getMyBidsStatusCargoOntheWayRequest({ filter: filterValues, sort: sortValues });
            if (res) {
                setTransports(res.data.map(v => v.clientAdvertisement))
            }
            setAdsLoading(false)
        } catch (error) {
            RequestError(error);
