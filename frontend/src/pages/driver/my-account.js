import { Button, Col, Collapse, Form, Input, InputNumber, List, Modal, Row, Select, Tabs, Upload } from 'antd';
...
import { getDriverByIdRequest } from '../../api/controllers/driver-controller';
import { getMyAddressesRequest, getSavedAddressByIdRequest, updateSavedAddressRequest } from '../../api/controllers/saved-address-controller';
...
const MyAccountDriver = () => {
    ...
    const editSavedAddressForm = {
        ...
        phone: selectedAddress?.phone?.slice(3),
    ...
    const fetchPerson = async () => {
        try {
            let res = await getDriverByIdRequest(currentUser.id);
            if (res) {
                setPerson(res.data);
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }} className="layout-content-padding">
            ...
            <Tabs defaultActiveKey="1" style={{ padding: "1rem 1.5rem", backgroundColor: "var(--white)", borderRadius: "1rem" }}>
                        ...
                        renderItem={item => (
                            <List.Item key={item.id}>
                                ...
                                <Button
                                    icon={<i style={{ marginRight: ".5rem" }} className="bi bi-pencil-square"></i>}
                                    type="primary"
                                    onClick={() => openAddressModal(item)}>{t("Düzenle")}</Button>
                            </List.Item>
                        )}
                    />
                </Tabs.TabPane>
...
export default MyAccountDriver;